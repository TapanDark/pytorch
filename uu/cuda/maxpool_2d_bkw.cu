...
namespace{    
  ...
  void max_pool2d_with_indices_backward_out_cuda_template(
    torch::Tensor& gradInput,
             ...
             bool ceil_mode)
  {
    std::cout<<"!! cuda max_pool2d_with_indices_backward_out_cpu_template inside\n";
...
torch::Tensor cmax_pool2d_with_indices_backward_cuda(
  const torch::Tensor& gradOutput_,
  ...
  const torch::Tensor& indices)
{
  std::cout<<"!! execute cuda here0 \n";
  auto gradInput = at::zeros_like(input, LEGACY_CONTIGUOUS_MEMORY_FORMAT);
  at::native::max_pool2d_with_indices_backward_out_cuda_template(
    gradInput,
    gradOutput_,
    input,
    indices,
    ...
    ceil_mode);
  return gradInput;
  std::cout<<"!! execute cuda here2 \n";
}



PYBIND11_MODULE(TORCH_EXTENSION_NAME, m) {
  m.def("backward", &cmax_pool2d_with_indices_backward_cuda, "cmax_pool2d cuda backward");
}
