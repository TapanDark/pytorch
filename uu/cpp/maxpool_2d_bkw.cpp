...
torch::Tensor& max_pool2d_with_indices_backward_out_cpu_template(
          ...
          bool ceil_mode)
{
  ...
  std::cout<<"!! max_pool2d_with_indices_backward_out_cpu_template inside\n";
...
torch::Tensor cmax_pool2d_with_indices_backward_cpu(
  const torch::Tensor& gradOutput_,
  const torch::Tensor& input,
  torch::IntArrayRef kernel_size,
  torch::IntArrayRef stride,
  ...
  const torch::Tensor& indices)
{
   std::cout<<"!! execute here0 \n";
  auto gradInput = at::zeros_like(input, LEGACY_CONTIGUOUS_MEMORY_FORMAT);
  at::native::max_pool2d_with_indices_backward_out_cpu_template(
    gradInput,
    gradOutput_,
    input,
    indices,
...
PYBIND11_MODULE(TORCH_EXTENSION_NAME, m) {
  m.def("backward", &cmax_pool2d_with_indices_backward_cpu, "cmax_pool2d backward");
}
